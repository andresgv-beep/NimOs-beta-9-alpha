...
func TestFormatBytes_Characterization(t *testing.T) {
	// CONVENCIÓN DE PRODUCTO (2026-07-04): división binaria con etiquetas
	// TB/GB, la convención NAS de facto (Synology/Windows/lsblk) que el
	// usuario espera. Un disco de "4 TB" de caja → "3.6 TB", igual que en
	// Synology. Idéntica base en formatters.js (frontend).
	cases := []struct {
		in   int64
		want string
	}{
		...
		{4000787030016, "3.6 TB"}, // el disco de 4 TB del sistema real, como en Synology
		{5368709120, "5.0 GB"},
	}
	for _, c := range cases {
