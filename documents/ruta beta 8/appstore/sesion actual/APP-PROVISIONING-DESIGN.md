...
> ┌─────────────────────────────────────────────────────────────────────┐
> │ MAPA DE DOCUMENTOS (leer en este orden):                            │
> │                                                                      │
> │  1. ESTE doc (APP-PROVISIONING-DESIGN) = el PORQUÉ y la VISIÓN      │
> │     · El problema, la idea, las capas (1 y 2), el flujo general.    │
> │     · Para ENTENDER el sistema y por qué existe.                    │
> │                                                                      │
> │  2. APP-CATALOG-SCHEMA.md = el CONTRATO EXACTO (más nuevo, 19/06)   │
> │     · Los campos precisos del catálogo (configVersion, configFields,│
> │       postInstall, secret, immutable, validation, auto, minVersion).│
> │     · La REFERENCIA al implementar · es la versión definitiva del   │
> │       esquema. Donde este doc y CATALOG difieran, MANDA CATALOG.    │
> │                                                                      │
> │  3. ENV-ESCAPE-HALLAZGO.md = el tema del $ en el .env (tests reales)│
> │     · Solo el $ rompe; se escapa $→$$ en valores de usuario.        │
> └─────────────────────────────────────────────────────────────────────┘

> NOTA: este documento es la visión inicial (18/06). Algunos detalles del
> esquema se REFINARON después en APP-CATALOG-SCHEMA.md (configApp eliminado y
> ahora se infiere; añadidos configVersion, minVersion, immutable, secret,
> validation, auto-como-objeto, id en postInstall). Para el contrato exacto,
> usar siempre APP-CATALOG-SCHEMA. Este doc se conserva por el CONTEXTO (el
> porqué, las capas, el flujo) que no está en el otro.
