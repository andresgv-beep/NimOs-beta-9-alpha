...
## 16. EXTERNAL SYSTEMS OWN THEIR FACTS (Source Of Truth)

**Añadido: 2026-06-08. Origen: incidente de drift de profile en Storage
(conversión single→raid1 por terminal que NimOS no reflejó). Auditoría
AUDIT-STOR-SOT.**

### El principio

Si un sistema externo es la **autoridad** de un dato, NimOS NO mantiene una
copia autoritativa de ese dato.

Puede **cachearla**. Puede **indexarla**. Puede **observarla**.
Pero la verdad sigue estando fuera.

### Autoridades reconocidas

Cada uno de estos sistemas es dueño de ciertos hechos. NimOS los lee, no los
posee:

| Sistema externo | Es autoridad de… |
|-----------------|------------------|
| **BTRFS** | profile (raid1/single…), composición de devices del pool, compresión, uso real, estado de balance/scrub |
| **Docker** | contenedores existentes, su estado, imágenes, redes, volúmenes |
| **systemd** | estado de servicios (active/failed), unidades, timers |
| **SMART** | salud física del disco, sectores, wear, temperatura |
| **kernel / udev** | paths de device (/dev/sdX, by-id), montajes activos, tipo de FS |
| **Caddy / red** | listeners activos, certificados emitidos, rutas servidas |

### La regla operativa

Para cada dato que NimOS muestra o usa, preguntar:

> **¿Quién es la autoridad de este hecho?**

- **Un sistema externo de la lista** → leer en vivo en cada consulta. La BD,
  si lo guarda, es caché que se **auto-corrige** contra la realidad (self-heal),
  nunca fuente de verdad.
- **NimOS mismo** (lo asignó, lo decidió, es metadato suyo) → la BD ES la
  autoridad. Correcto persistir. Ej: `id`, `name`, `role`, `control_state`,
  nombres asignados, intención del usuario.

### Cómo distinguir (test rápido)

> *"Si alguien cambia esto por fuera de NimOS (terminal, script, otra
> herramienta), ¿NimOS debería reflejarlo?"*

- **Sí** → es de un sistema externo. Leer en vivo. (profile, devices, servicios)
- **No / no tiene sentido cambiarlo por fuera** → es de NimOS. Cachear en BD.
  (el nombre que el user puso al pool, el rol asignado)

### Por qué importa (no es teórico)

El bug que originó esta regla apareció en Storage, pero la **misma clase** acecha
en todos los módulos que reflejan estado externo:

- **Docker**: NimOS lista contenedores de su BD → alguien hace `docker rm` por
  CLI → NimOS muestra un contenedor fantasma. Mismo bug.
- **systemd**: NimOS cree que un servicio está activo porque lo arrancó → el
  servicio murió → NimOS miente sobre su estado.
- **Network**: NimOS cree que expone un puerto → Caddy se reconfiguró por fuera
  → divergencia.
- **SMART**: NimOS cachea "disco sano" → el disco se degradó desde el último
  scan → falsa tranquilidad.

No es un problema de Storage. Es un problema de **SOT (Source Of Truth)**
transversal. Esta regla lo previene en TODOS los módulos, presentes y futuros.

### Patrón de implementación

```
GetX():
    base   := repo.GetX()          // metadatos propios de NimOS (BD)
    real   := readFromAuthority()  // hechos del sistema externo (en vivo)
    merged := reconcile(base, real) // real gana en los campos que posee
    if merged diverge de base:
        go persistInBackground(merged)  // self-heal, no bloquea
    return merged
```

La BD nunca es la última palabra sobre un hecho externo. Es un punto de partida
que se valida contra la realidad en cada lectura.

### Excepción legítima

Hechos externos **inmutables** pueden cachearse sin re-lectura constante:
`btrfs_uuid` (no cambia nunca), `serial` de disco (firmware). Si el dato físico
no puede cambiar, el caché no puede derivar.
